    ...
    this.actions$.pipe(
      ...
      switchMap(() => {
        ...
        const startTime = Number(
          localStorage.getItem('cr-stopwatch-starttime'),
        );
        ...
        return of(
          StopwatchActions.restoreSuccess({
          ...
          switch (status) {
            case StopwatchStatus.Running:
              localStorage.setItem(
                'cr-stopwatch-starttime',
                startTime.toString(),
              );
              this.intervalId = window.setInterval(() => {
                this.store$.dispatch(
                  StopwatchActions.tick({ now: Date.now() }),
                );
              }, 100);
              break;
    ...
    () =>
      this.actions$.pipe(
        ...
        tap(() => {
          ...
          localStorage.setItem(
            'cr-stopwatch-status',
            StopwatchStatus.NotStarted.toString(),
          );
